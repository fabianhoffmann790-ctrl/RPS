...
export function Timeline({ title, blocks, ui, onUiChange, showControls = false }: Props) {
  ...
  const { windowStart, windowEnd, durationMs, widthPx } = useMemo(() => {
    ...
    start.setHours(ui.startHour, 0, 0, 0);

    const endBaseline = ui.endHour <= ui.startHour ? addDays(baseline, 1) : baseline;
    const end = new Date(endBaseline);
    end.setHours(ui.endHour, 0, 0, 0);

    const minBlockStart = blocks.length
      ? Math.min(...blocks.map((block) => new Date(block.start).getTime()))
      : undefined;
    const maxBlockEnd = blocks.length ? Math.max(...blocks.map((block) => new Date(block.end).getTime())) : undefined;

    const windowStart = minBlockStart ? Math.min(start.getTime(), minBlockStart) : start.getTime();
    const windowEnd = maxBlockEnd ? Math.max(end.getTime(), maxBlockEnd) : end.getTime();
    const durationMs = Math.max(1, windowEnd - windowStart);
    const hours = durationMs / (1000 * 60 * 60);
    const widthPx = Math.max(900, Math.round(hours * pxPerHourByZoom[ui.zoomMinutes]));

    return { windowStart, windowEnd, durationMs, widthPx };
  }, [blocks, ui.endHour, ui.startHour, ui.zoomMinutes]);
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div ref={scrollRef} className="overflow-x-auto rounded-lg border border-slate-200 bg-slate-50">
        <div className="relative h-28 p-2" style={{ width: `${widthPx}px` }}>
          {ui.showGrid
            ? Array.from({ length: Math.ceil(durationMs / (ui.zoomMinutes * 60_000)) + 1 }).map((_, index) => {
                const x = (index * ui.zoomMinutes * 60_000 * widthPx) / durationMs;
          ...
          {blocks.map((block) => {
            ...
            const left = ((start - windowStart) / durationMs) * widthPx;
            const width = Math.max(24, ((end - start) / durationMs) * widthPx);
            return (
              <div
                key={block.id}
